...
const { title, tagLine, carouselData } = pojectOneData
const settings = {
    items: 1,
    gutter: 30,
    loop: false,
        ...
        <section className="project-one">
            <Container >
                ...
                <TinySlider settings={settings} className="project-one__carousel">
                    {
                        carouselData.map(({ id, image, title, href, cats, catsHref }) => <div key={id} className="item">
                            <div className="project-one__item">
                                <div className="project-one__image">
                                    <Image src={image} alt="tolak" />
                                </div>
                                <div className="project-one__hover">
                                    <Link className="project-one__read-more" href={href}><span className="icon-right-arrow"></span></Link>
                                    <div className="project-one__hover-bottom">
                                        <div className="project-one__cats"><Link href={catsHref}>{cats}</Link></div>
                                        <h3 className="project-one__title"><Link href={href}>{title}</Link></h3>
                                    </div>
                                </div>
                            </div>
                        </div>)
